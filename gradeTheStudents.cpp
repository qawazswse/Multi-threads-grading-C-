...
//									   //
//  (1) How did your program behave when not thread-safe?		   //

        //Answer: Sometime when a line is not finished printing, another print shows right after it. Which make the output unreadable.
//									   //
//									   //
//									   //
//(2) How did your program behave when threads-safe but output not guarded?//

        //Answer: Same kind of sentences don't get together, the output looks not pretty.
//									   //
//									   //
//									   //
//(3) How did your program behave when threads-safe and output not guarded?//

        //Answer: Same kind of sentences like to show together, easier to read and looks better.
//									   //
//									   //
//									   //
//---									---//
...
//	get the graded test out of their personal test buffer, comments
//	on it (see function 'commentOnTest()'), and 'free()'s the Test ptr.
//	IT MUST 'free()' IT OR THERE IS A MEMORY LEAK!
//
//	Returns 'vPtr'.
...
{
  //  YOUR CODE HERE
  struct student* student1 = (struct student*)vPtr;
  for (int i = 0; i < NUM_TESTS_PER_STUDENT; i++)
  {
    Test* t = takeTest(student1->id);
    student1->tTB->putIn(t);
  }
  for (int i = 0; i < NUM_TESTS_PER_STUDENT; i++)
  {
    Test* t1 = student1->sTBA->pullOut();
  pthread_mutex_lock(&lock);
    printf("student %d : %s\n", t1->studentId_, commentOnTest(t1));
  pthread_mutex_unlock(&lock);
    free(t1);
  }
  return(vPtr);
...
{
  //  YOUR CODE HERE
  struct grade* grade1 = (struct grade*)vPtr;
  for (int i = 0; i < NUM_TESTS_PER_GRADER; i++)
  {
    Test* t1 = grade1->tTB->pullOut();
  	gradeTest(t1);
    grade1->gTB->putIn(t1);
  }
  return(vPtr);
...
{
  //  YOUR CODE HERE
  struct professor* professor1 = (struct professor*)vPtr;
  for(int i = 0; i < NUM_TESTS; i++) 
  {
    Test* t1 = professor1->gTB->pullOut();
  pthread_mutex_lock(&lock);
    printf("student id: %d, grade: %d\n", t1->studentId_, t1->total_);
  pthread_mutex_unlock(&lock);
    professor1->sTBA[t1->studentId_].putIn(t1);
  }
  return(vPtr);
}
...
int		main		()
{
  ...
  professor1 = (struct professor*)malloc(sizeof(struct professor));
  ...
  professor1->gTB = &gradedTestBuffer;
  professor1->sTBA = studentTestsBufferArray;
  //  (3) starting thread 'profThreadId' to do 'doProfessor' given your struct
  pthread_create(&profThreadId,NULL,doProfessor,(void*)professor1);

  for  (int i = 0;  i < NUM_GRADERS;  i++)
  {
    //  YOUR CODE HERE TO:
    //  (1) malloc() memory for struct for doGrader
    grade1 = (struct grade*)malloc(sizeof(struct grade));
    //  (2) filling in the member vars:
    //      (a) the address of takenTestBuffer
    //      (b) the address of gradedTestBuffer
    grade1->tTB = &takenTestBuffer;
    grade1->gTB = &gradedTestBuffer;
    //  (3) starting thread 'graderThreadIdArray[i]' to do 'doGrader' given
    //	    your struct
    pthread_create(&graderThreadIdArray[i],NULL,doGrader,(void*)grade1);
  } 
  ...
  for  (int i = 0;  i < NUM_STUDENTS;  i++)
  {
    ...
    student1->tTB = &takenTestBuffer;
    student1->sTBA = &studentTestsBufferArray[i];
    student1->id = i;
    //  (3) starting thread 'studentThreadIdArray[i]' to do 'doStudent' given
    //	    your struct
    pthread_create(&studentThreadIdArray[i],NULL,doStudent,(void*)student1);
  }
  ...
  for  (int i = 0;  i < NUM_STUDENTS;  i++)
  {
    ...
    pthread_join(studentThreadIdArray[i],(void**)&student1);
  ...
  for  (int i = 0;  i < NUM_GRADERS;  i++)
  {
    ...
    pthread_join(graderThreadIdArray[i],(void**)&grade1);
    //	(b) 'free()' the address of the struct that it gives back
    free(grade1);
  }
  ...
  pthread_join(profThreadId,(void**)&professor1);
  //  (b) 'free()' the address of the struct that it gives back
  free(professor1);
